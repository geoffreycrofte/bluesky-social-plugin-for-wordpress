...
<objective>
Write PHPUnit tests for the 4 highest-priority areas: AJAX security, syndication logic, settings sanitization, and Helpers utilities.

Purpose: Test coverage for critical paths ensures the refactoring preserved correct behavior and security fixes are verified. Priority order per research: AJAX (security) > Syndication (business value) > Settings (user input) > Helpers (utilities).
Output: Test suite with 15-25 tests covering critical paths.
</objective>
...
<task type="auto">
  <name>Task 1: Write AJAX and Syndication service tests</name>
  <files>tests/unit/BlueSky_AJAX_Service_Test.php, tests/unit/BlueSky_Syndication_Service_Test.php, tests/bootstrap.php</files>
  <action>
...
Run `./phpunit.phar --configuration phpunit.xml` after creating both test files.
  ...
  <verify>Run `./phpunit.phar --configuration phpunit.xml` -- all tests pass (11+ tests, 0 failures). If any fail, fix the test or the code issue.</verify>
  <done>AJAX service tests verify nonce checks on all endpoints and auth gating. Syndication tests verify post status guards, capability checks, and sanitization. All pass.</done>
</task>
...
<verification>
- `./phpunit.phar --configuration phpunit.xml` shows 20+ tests, 0 failures
- Test coverage hits AJAX, Syndication, Settings sanitize, Helpers (the 4 research priorities)
- No test requires a real WordPress installation or database
- All tests use Brain Monkey for WP function mocking
</verification>

<success_criteria>
PHPUnit test suite covers the 4 highest-priority areas with 20+ passing tests. Security fixes (nonce, capability, sanitization) are verified by tests. No WordPress install required to run tests.
</success_criteria>

...
<task type="auto">
  ...
  <action>
Update `social-integration-for-bluesky.php` to load and instantiate the Account Manager as a global variable.
...
4. Do NOT modify the `BlueSky_Plugin_Setup` constructor or pass `$bluesky_account_manager` to it. That wiring is owned by Plan 02 Task 1. This task ONLY adds the require_once and the `$bluesky_account_manager = new BlueSky_Account_Manager()` instantiation line.

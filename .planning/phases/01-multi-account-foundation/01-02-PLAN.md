...
Add a `private $account_manager;` property to the class. Update the constructor signature to accept the Account Manager: `__construct(BlueSky_API_Handler $api_handler, BlueSky_Account_Manager $account_manager = null)`. Store it: `$this->account_manager = $account_manager;`. The null default preserves backward compatibility.

Then update `social-integration-for-bluesky.php` to pass the account manager to Plugin Setup. Change the existing instantiation from `new BlueSky_Plugin_Setup($bluesky_api_handler)` to `new BlueSky_Plugin_Setup($bluesky_api_handler, $bluesky_account_manager)`. The `$bluesky_account_manager` variable was created by Plan 01 Task 2.

**2. Register the enable_multi_account setting:**
In the existing `sanitize_callback` for the `bluesky_settings` option (inside `register_settings()`), add explicit handling for `enable_multi_account`: cast it to `(bool)` — `$input['enable_multi_account'] = !empty($input['enable_multi_account']);`. This ensures the field is always stored as a boolean in the settings array. The field name in the option array: `bluesky_settings[enable_multi_account]`.
...
Grep: `grep 'private.*account_manager' classes/BlueSky_Plugin_Setup.php` — Account Manager property declared.
Grep: `grep '__construct.*BlueSky_Account_Manager' classes/BlueSky_Plugin_Setup.php` — constructor accepts Account Manager.
Grep: `grep 'check_admin_referer.*bluesky_add_account' classes/BlueSky_Plugin_Setup.php` — nonce verified for add account.
Grep: `grep 'check_admin_referer.*bluesky_remove_account' classes/BlueSky_Plugin_Setup.php` — nonce verified for remove account.
Grep: `grep 'check_admin_referer.*bluesky_switch_account' classes/BlueSky_Plugin_Setup.php` — nonce verified for switch account.
Grep: `grep 'BlueSky_Plugin_Setup.*bluesky_account_manager' social-integration-for-bluesky.php` — account manager passed to Plugin Setup constructor.
  </verify>
  <done>

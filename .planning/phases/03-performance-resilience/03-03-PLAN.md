...
**Action Scheduler bundling:** Check if Action Scheduler is already available (WooCommerce or other plugin provides it). If not, the plugin will gracefully fall back to synchronous execution. Do NOT bundle Action Scheduler in this plan — we use function_exists() guard for graceful degradation. A future plan or admin notice can recommend installing it.
  ...
  <verify>File exists with all methods. PHP syntax check: `/Users/CRG/Library/Application Support/Local/lightning-services/php-8.3.8+0/bin/darwin/bin/php -l classes/BlueSky_Async_Handler.php`. Verify both code paths: (1) grep for `as_schedule_single_action` confirming Action Scheduler path exists, and (2) grep for `function_exists.*as_schedule_single_action` confirming the fallback guard exists. Read the schedule_syndication method and verify the if/else branches for both paths.</verify>
  <done>BlueSky_Async_Handler.php exists with schedule_syndication, process_syndication, retry logic, circuit breaker integration, and graceful fallback when Action Scheduler unavailable. Both code paths verified: Action Scheduler present path uses as_schedule_single_action, absent path falls back to synchronous syndication.</done>
</task>

<task type="auto">
  <name>Task 2: Modify Syndication_Service to delegate to async handler</name>
  <files>classes/BlueSky_Syndication_Service.php, classes/BlueSky_Plugin_Setup.php</files>
  <action>
...
**Wiring in Plugin_Setup.php constructor (classes/BlueSky_Plugin_Setup.php):**
The constructor currently instantiates services at lines ~54-59 following this pattern:
```php
$this->syndication = new BlueSky_Syndication_Service($api_handler, $account_manager);
```
Add async handler instantiation BEFORE the syndication service line:
```php
...
```
Also add `classes/BlueSky_Plugin_Setup.php` to files modified for this task.
  </action>
  <verify>PHP syntax check on modified files (Syndication_Service.php and Plugin_Setup.php). Verify existing syndication validation logic is preserved by reading the file. Verify Plugin_Setup.php constructor creates BlueSky_Async_Handler and passes it to Syndication_Service.</verify>

...
must_haves:
  truths:
    - "After 3 consecutive API failures for an account, is_available() returns false for that account"
    - "After 15-minute cooldown expires, circuit transitions to half-open and allows one test request"
    - "Successful request in half-open state closes the circuit"
    - "HTTP 429 response is detected and Retry-After header value is extracted"
    - "When Retry-After header is absent, exponential backoff calculates 60s/120s/300s with jitter"
...
- Backoff includes jitter (result varies within ±20% of base delay)

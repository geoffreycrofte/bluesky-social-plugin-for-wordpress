...
must_haves:
  ...
  key_links:
    ...
    - from: "classes/BlueSky_Render_Front.php"
      to: "templates/frontend/stale-indicator.php"
      via: "include statement in render methods when cache is stale"
      pattern: "include.*stale-indicator\\.php"
---
...
8. CACHE DURATION RETRIEVAL: Get cache TTL via `$this->options['cache_duration']['total_seconds'] ?? 600` (per user decision: reuse existing admin settings cache duration option, default to 10 minutes / 600 seconds if not set). The existing `$this->options` is populated from `get_option('bluesky_settings')` which already contains `cache_duration.total_seconds` from the admin settings page.

9. STORE IN CACHES: Store result in both transient cache (with extended TTL for stale fallback: cache_duration * 2) AND request cache. Also set a freshness marker transient ({cache_key}_fresh) with normal cache_duration TTL.
...
3. If data is stale (freshness expired but data exists):
   ...
   - Include "last updated X ago" indicator by explicitly including the template:
     ```php
     $time_ago = BlueSky_Helpers::time_ago($cache_timestamp);
     include plugin_dir_path(dirname(__FILE__)) . 'templates/frontend/stale-indicator.php';
     ```
   - The stale-indicator template expects `$time_ago` variable to be set before include
